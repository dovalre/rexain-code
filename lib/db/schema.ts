...
import { pgTable, text, timestamp, boolean, index, uniqueIndex, uuid, varchar, json, numeric } from "drizzle-orm/pg-core";
...
export const user = pgTable("user", {
  ...
  image: text("image"),
  credit: numeric("credit", { precision: 12, scale: 6 }).default("0").notNull(),
...
export const creditTransaction = pgTable(
  "credit_transaction",
  {
    ...
    userId: text("userId")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    type: varchar("type", { enum: ["topup", "usage"] }).notNull(),
  ...
  (table) => [
    ...
    uniqueIndex("credit_transaction_generation_id_unique_idx")
